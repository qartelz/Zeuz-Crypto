  ...
  if (onClose) {
    return (
      <>
        ...
        {/* Mobile Sidebar */}
        <div
          className={`fixed top-0 left-0 h-full w-[280px] z-50 bg-[#0a0a14] transition-transform duration-300 ${
            isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
        >
          <div className="flex flex-col h-full py-6 bg-[radial-gradient(rgba(0,0,0,0.6),transparent_70%)] backdrop-blur-md border-r border-white/10">
            {/* Close Button */}
            <div className="flex justify-end px-4 mb-4">
              <button
                onClick={onClose}
                className="p-2 rounded-lg hover:bg-[#D643BF]/10 transition-colors"
              >
                <X className="text-[#D643BF] w-6 h-6" />
              </button>
            </div>
            ...
            <div className="flex flex-col flex-1 px-4 space-y-2">
              {navItems.map(({ id, icon: Icon, label, path }) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center space-x-3 py-3 px-4 rounded-lg cursor-pointer transition-all duration-200 ${
                      isActive ? "bg-[#D643BF]/20" : "hover:bg-[#D643BF]/10"
                    }`
                  }
                >
            ...
            <div className="flex flex-col px-4 space-y-2">
              {bottomItems.map(({ id, icon: Icon, label }) => (
                <button
                  ...
                  onClick={handleLogout}
                  disabled={loggingOut}
                  className={`flex items-center space-x-3 py-3 px-4 rounded-lg cursor-pointer transition-all duration-200 w-full text-left ${
                    loggingOut
                      ? "bg-gray-600 cursor-not-allowed"
                      : "hover:bg-[#D643BF]/10"
                  }`}
