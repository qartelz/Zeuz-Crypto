...
//   //       holding_type: tradeType.toUpperCase(),
//   //       quantity: amount,
...
const Trading = () => {
  ...
  //   try {
  //     // Mock token check
  //     const tokens = { access: 'mock-token' };
  ...
  //       trade_type: mode === "spot" ? "SPOT" : "FUT",
  //       direction: spotSide.toUpperCase(),
  //       holding_type: mode === "spot" ? "LONGTERM" : holdingType.toUpperCase(),
  //       quantity: amount,
  //       price: fmt(ticker?.c, 2),
  //       order_type: "MARKET",
  //     };
  ...
  //     // Mock API call
  //     showToast(`Order placed: ${spotSide.toUpperCase()} ${amount} ${selected.baseAsset} @ ${fmt(ticker?.c, 2)}`, 'success');
      ...
      if (spotSide === "sell") {
        ...
        if (isFull) {
          ...
          response = await fetch(
            `http://127.0.0.1:8000/api/v1/trading/close-trade/${tradeId}/`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          response = await fetch(
            `http://127.0.0.1:8000/api/v1/trading/partial-close/${tradeId}/`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
      ...
      {tradeType === "Order Book" ? (
        ...
        <>
          {mode === "futures" ? (
            ...
            //       <div className="text-xs text-gray-400">
            //         Available: <span>{spotBalance.toFixed(8)} {selected.baseAsset}</span>
            //       </div>
            ...
            <>
              ...
              {spotSide === "buy" ? (
                ...
                <div className="space-y-3">
                  ...
                  <button
                    onClick={handleOrder}
                    disabled={isPlacingOrder || !tradeId || spotBalance === 0}
                    className={`w-full py-2 rounded-lg font-semibold transition 
                    ${
                        !tradeId || spotBalance === 0 || isPlacingOrder
  ...
  const fetchSpotBalance = async () => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/trading/trade/open/?asset_symbol=${selected.baseAsset}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
