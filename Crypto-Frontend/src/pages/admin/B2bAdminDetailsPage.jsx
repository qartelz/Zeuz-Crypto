...
//   return (
//     <div className="min-h-screen text-white p-6 bg-[#0F051A]">
//       {toast && (
//         <Toast
//           message={toast.message}
//           type={toast.type}
...
//         {/* --- REFACTORED: Admin Details Header --- */}
//         <div className="rounded-lg border border-purple-500/30 bg-[#160C26] overflow-hidden mb-8 shadow-lg shadow-purple-900/10">
//           <div className="p-6">
//             <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
//               <div className="flex items-center gap-4">
//                 <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-600 to-indigo-600 flex-shrink-0 flex items-center justify-center text-2xl font-bold">
//                   {adminDetails.user.first_name?.[0] || 'A'}
//                 </div>
//                 <div>
...
//                 <div>
//                   <p className="text-xs text-purple-300/70">Email</p>
//                   <p className="font-medium text-white text-sm truncate">{adminDetails.user.email}</p>
//                 </div>
//               </div>
//               <div className="flex items-center gap-2">
...
//               </div>
//               <div className="flex items-center gap-2">
//                 <Wallet size={16} className="text-emerald-400 flex-shrink-0" />
//                 <div>
//                   <p className="text-xs text-purple-300/70">Wallet Balance</p>
//                   <p className="font-medium text-emerald-400 text-sm">
//                     ${parseFloat(adminDetails.user.wallet.balance).toLocaleString()}
...
//         {/* --- REFACTORED: Tabs --- */}
//         {!selectedBatch && (
//           <div className="mb-6 rounded-lg w-96 border border-purple-500/20 bg-[#160C26] overflow-hidden">
//             <div className="flex space-x-0">
//               <button
//                 onClick={() => setActiveTab('batches')}
//                 className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 font-semibold transition-all duration-300 ${
//                   activeTab === 'batches'
...
//               value={searchQuery}
//               onChange={(e) => setSearchQuery(e.target.value)}
//               className="w-full pl-12 pr-4 py-3 bg-[#160C26] border border-purple-500/30 rounded-lg text-white placeholder-purple-300/50 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500"
//             />
//           </div>
//         </div>
...
//               </div>
//               <div className="text-right">
//                 <p className="text-sm text-purple-300/70 uppercase tracking-wider">Total Users</p>
//                 <p className="text-3xl font-bold text-white">{selectedBatch.user_count}</p>
//               </div>
//             </div>

//             <div className="rounded-lg border border-purple-500/20 bg-[#160C26] overflow-hidden">
//               <div className="overflow-x-auto">
//                 <table className="w-full">
//                   <thead className="bg-purple-500/10">
//                     <tr>
//                       <th className="px-6 py-4 text-left text-xs font-semibold text-purple-300 uppercase tracking-wider">Name</th>
//                       <th className="px-6 py-4 text-left text-xs font-semibold text-purple-300 uppercase tracking-wider">Email</th>
//                       <th className="px-6 py-4 text-left text-xs font-semibold text-purple-300 uppercase tracking-wider">Mobile</th>
//                       <th className="px-6 py-4 text-left text-xs font-semibold text-purple-300 uppercase tracking-wider">Wallet Balance</th>
...
//           </div>
//         ) : activeTab === 'batches' ? (
//           // --- REFACTORED: Batches View (like Frame 19.png) ---
//           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
//             {filteredBatches.map((batch) => (
//               <div
//                 key={batch.id}
//                 onClick={() => handleBatchClick(batch)}
//                 className="group rounded-lg bg-[#160C26] p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10 cursor-pointer"
//               >
//                 {/* Top content */}
//                 <div>
...
//                       {batch.name}
//                     </h3>
//                     <ChevronRight className="text-purple-400 group-hover:translate-x-1 transition-transform flex-shrink-0" size={24} />
//                   </div>
//                   <span className="inline-block bg-purple-500/20 text-purple-300 text-xs font-medium px-2.5 py-1 rounded">
//                     {batch.user_count} / {batch.max_users} users
//                   </span>
//                   <p className="text-purple-300/80 text-sm mt-3 mb-4 line-clamp-2">{batch.description}</p>
//                 </div>
//                 {/* Footer content */}
...
//           </div>
//         ) : (
//           // --- REFACTORED: Users View ---
//           <div className="rounded-lg bg-[#160C26] overflow-hidden">
//             <div className="overflow-x-auto">
//               <table className="w-full">
//                 <thead className="bg-purple-500/10">
//                   <tr>
...
const B2bAdminDetailsPage = () => {
  ...
  return (
    <div className="min-h-screen text-white p-6 rounded-3xl bg-[#0F051A]">
      {toast && (
        <Toast
          message={toast.message}
          type={toast.type}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="rounded-lg border border-purple-500/30 bg-[#160C26] overflow-hidden mb-8 shadow-lg shadow-purple-900/10">
          <div className="p-5">
            <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-purple-600 to-indigo-600 flex-shrink-0 flex items-center justify-center text-xl font-bold">
                  {adminDetails.user.first_name?.[0] || 'A'}
                </div>
                <div>
        ...
        {/* --- REFACTORED: Tabs --- */}
        {!selectedBatch && (
          <div className="mb-6 flex space-x-6 border-b border-purple-500/30">
            <button
              onClick={() => setActiveTab('batches')}
              className={`relative flex items-center gap-2 py-3 px-2 font-semibold transition-colors duration-200 ${
                activeTab === 'batches'
                  ? 'text-white'
            ...
            >
              <Briefcase size={18} />
              <span className="text-sm">Batches</span>
              {activeTab === 'batches' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-500 rounded-t-full"></div>}
            </button>
            <button
              onClick={() => setActiveTab('users')}
              className={`relative flex items-center gap-2 py-3 px-2 font-semibold transition-colors duration-200 ${
            ...
            >
              <Users size={18} />
              <span className="text-sm">Users</span>
              {activeTab === 'users' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-500 rounded-t-full"></div>}
            </button>
          </div>
        )}
        ...
        <div className="mb-6 max-w-md">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-purple-300/70" size={18} />
            <input
              type="text"
              placeholder={`Search ${selectedBatch ? 'batch users' : activeTab}...`}
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-11 pr-4 py-2.5 bg-transparent border border-purple-500/30 rounded-lg text-white placeholder-purple-300/50 focus:outline-none focus:border-purple-500"
            />
          </div>
        </div>
            ...
            <div className="rounded-lg  overflow-hidden">
              ...
              {batchUsersNext && !batchUsersLoading && (
                <div className="p-6 text-center border-t border-purple-500/20">
                  <button
                    onClick={() => fetchBatchUsers(selectedBatch.id, batchUsersPage + 1)}
                    className="px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 rounded-lg font-semibold transition-all"
                  >
                    Load More
                  </button>
                </div>
