  ...
  const CreateChallenge = ({ onBack, onSuccess }) => {
    ...
    const handleCreateWeek = async (weekNumber) => {
      ...
      if (!week) {
        ...
        console.error(`Week ${weekNumber} not found in weeks array`, { weekNumber, weeks });
        return null;
      }
    
      // Check if week is already created
      if (createdWeeks[weekNumber]) {
        console.log(`Week ${weekNumber} already created with ID:`, createdWeeks[weekNumber].id);
      ...
      try {
        ...
        console.log(`Creating Week ${weekNumber} Payload:`, JSON.stringify(payload, null, 2));
        ...
        console.log(`Week ${weekNumber} Response Data:`, JSON.stringify(data, null, 2));
        ...
        if (response.ok) {
          ...
          showToast(`Failed to create week ${weekNumber}: ${data.detail || JSON.stringify(data)}`, "error");
          return null;
        }
      } catch (error) {
    ...
    <div className="min-h-screen text-white p-6">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="rounded-2xl border border-purple-500/30 p-8">
          <h1 className="text-3xl font-bold mb-6 flex items-center gap-3">
            <Plus className="text-purple-400" size={32} />
            {step === 1 ? 'Create New Challenge' : step === 2 ? 'Add Weeks (Max 4)' : 'Add Tasks (1 per Week)'}
          </h1>
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4 mb-6">
                {[1, 2, 3, 4].map((weekNum) => {
                  const isPreviousWeekSaved = weekNum === 1 || weeks[weekNum - 2];
                  ...
                  return (
                    <div key={weekNum} className="relative h-20">
                      ...
                      >
                        <div className={`text-lg font-bold ${isLocked ? 'text-gray-500' : 'text-white'}`}>
                          Week {weekNum}
                        </div>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-gray-900/80 backdrop-blur-sm rounded-xl border border-gray-700/50">
                          <div className="text-center px-2">
                            ...
                            <div className="text-xs text-gray-400 font-medium leading-tight">
                              Save Week {weekNum - 1}<br />to add Week {weekNum}
                            </div>
                          </div>
                        </div>
                      )}
      ...
      {weeks.map((week, index) => {
        ...
        return (
          ...
          >
            ...
            {isCreated && (
              <div className="text-xs text-green-400 mt-1">✓ Created</div>
            )}
            {!isCreated && (
              <div className="text-xs text-yellow-400 mt-1">Not created</div>
            )}
          </button>
        );
      })}
...
const handleDeleteTask = async (taskId) => {
  ...
  try {
    ...
    const response = await fetch(
      `${baseURL}challenges/tasks/${taskId}/`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${tokens.access}`,
    ...
    if (response.ok || response.status === 204) {
      ...
      const data = await response.json();
      showToast(`Failed to delete task: ${data.detail || "Unknown error"}`, "error");
    }
  } catch (error) {
...
const handleCancelEdit = () => {
  ...
  const currentWeekTasks = currentWeek && weekTasks[currentWeek.id] 
    ? weekTasks[currentWeek.id] 
    : [];
        ...
        <div key={task.id}>
          ...
          {editingTask && editingTask.id === task.id ? (
            <div className="rounded-lg p-4 border border-purple-500/30 bg-purple-900/20">
              <h4 className="font-semibold text-white mb-3">Edit Task</h4>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full border border-purple-500/30 rounded-lg px-3 py-2 text-white bg-transparent focus:outline-none focus:border-purple-500 min-h-[80px]"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditTaskData({ ...editTaskData, task_type: e.target.value })
                      }
                      className="w-full border border-purple-500/30 rounded-lg px-3 py-2 text-white bg-transparent focus:outline-none focus:border-purple-500"
                    >
                      <option value="PORTFOLIO_BALANCE" className="bg-gray-900">Portfolio Balance</option>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditTaskData({ ...editTaskData, target_value: parseFloat(e.target.value) })
                      }
                      className="w-full border border-purple-500/30 rounded-lg px-3 py-2 text-white bg-transparent focus:outline-none focus:border-purple-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditTaskData({ ...editTaskData, is_mandatory: e.target.value === "true" })
                      }
                      className="w-full border border-purple-500/30 rounded-lg px-3 py-2 text-white bg-transparent focus:outline-none focus:border-purple-500"
                    >
                      <option value="true" className="bg-gray-900">Yes</option>
