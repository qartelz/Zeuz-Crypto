...
class SubscriptionOrder(models.Model):
    ...
    def clean(self):
        ...
        if hasattr(self.user, 'profile') and getattr(self.user.profile, 'user_type', None) == 'B2B':
            if self.order_type == 'SELF_PURCHASED':
                raise ValidationError("B2B users cannot self-purchase plans. Orders must be admin assigned.")
        
        # Validate coupon is applicable to plan
        if self.coupon and self.plan:
            if not PlanCoupon.objects.filter(plan=self.plan, coupon=self.coupon).exists():
    ...
    def complete_order(self, force=False):
        ...
        with transaction.atomic():
            ...
            if self.status in ['FAILED', 'CANCELLED']:
                raise ValidationError(f"Cannot complete order with status: {self.status}")
    ...
    def cancel_order(self, reason=None):
        ...
        # Log cancellation
        if hasattr(self, '_subscription') or Subscription.objects.filter(
...
class Subscription(models.Model):
    ...
    def __str__(self):
        return f"{self.user.username} - {self.plan.name} ({self.status}) - {self.subscription_source}"
    ...
    def save(self, *args, **kwargs):
        ...
        elif not is_new and previous_values != {k: v for k, v in new_values.items() if k in previous_values}:
